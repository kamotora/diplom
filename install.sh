...
CREATE USER '${dbuser}'@'localhost' IDENTIFIED BY PASSWORD('${dbpass}');
...
function change_app_config(){
	...
	# Проверяем, известны ли данные от БД
...
while true; do
	read -p "Вы хотите установить jdk (Нужно: jdk 11 или выше)? [y/n]: " yn
	case $yn in
		[Yy]* ) install_jdk;break;;
		[Nn]* ) break;;
		* ) echo "Введите y или n";;
	esac
done
...
# Настройка mysql
while true; do
	read -p "Вы хотите установить mysql, создать пользователя mysql и базу данных для приложениия? [y/n]: " yn
	case $yn in
		[Yy]* ) install_mysql;mysql-settings;break;;
		[Nn]* ) break;;
		* ) echo "Введите y или n";;
	esac
done
...
# Настройка app.jar
while true; do
	read -p "Вы хотите изменить конфигурацию приложения app.jar? [y/n]: " yn
	case $yn in
		[Yy]* ) change_app_config;break;;
		[Nn]* ) break;;
		* ) echo "Введите y или n";;
	esac
done
...
# Настройка nginx
while true; do
	read -p "Вы хотите установить и настроить конфигурацию nginx? [y/n]: " yn
	case $yn in
		[Yy]* ) apt-get install nginx -y;nginx-settings;break;;
		[Nn]* ) break;;
		* ) echo "Введите y или n";;
	esac
done
